...
import { useMovieInfo, useMovieData } from "../hooks/useMovies";
...
/*export const HandleWishlist = () => {
    ...
    dispatch(addMovieInWishList(movieInfo))
    if(!user) return
    
    const userRef = doc(db, 'users', user);
    const userSn = getDoc(userRef)

    if (userSn?.exists()) {
        //console.log("Document data:", docSnap.data());
        
        const info = userSn?.data()
        console.log(info)
    } else {
...
const MovieInfo = () => {
  ...
  //const stateWishList = useSelector(store => store.movie.wishList)
  const wish = useSelector((store) => store?.user?.wish);
  const user = useSelector((store) => store?.user?.uid);

  const dispatch = useDispatch();
  const imgUrl =
    "https://media.istockphoto.com/id/1337144146/ro/vector/implicit-avatar-profil-icon-vector.jpg?s=2048x2048&w=is&k=20&c=J9_blvGXaq_t7qrhr8Pq_veC7dDo9dZ6bjRU730GWzI=";
  // if(!info) return
  //console.log(info)
  ...
  const runtime = Number(movieInfo?.runtime);
  ...
  const HandleWishlist = async () => {
    //console.log(movieInfo)
    /* const StringMovieInfo = JSON.stringify(movieInfo)
        const existingWishlist = JSON.parse(localStorage?.getItem(user))*/

    /*  localStorage.setItem(user, StringMovieInfo)
        if(existingWishlist) console.log("truthy")
            else console.log("falsy") */

    /*
        if(existingWishlist) {
           // const arr2 = [JSON.parse(existingWishlist)]
            const list = [...existingWishlist, {...movieInfo}]
    ...
    /* const userRef = doc(db, 'users', user);
        const userSn = await getDoc(userRef)
        if (userSn?.exists()) {
            //console.log("Document data:", docSnap.data());
             updateDoc(userRef, {
                wishlist: wishList
  ...
  if(!rottonRatings) return
  const rottonRatingsValue = rottonRatings[0]?.Value
  
  

  //console.log(processedRuntime)
    ...
    <div className=" bg-gray-800 w-[100%] h-auto absolute">
      <div className=" mx-[150px] mt-[100px]">
        <div className="  flex justify-between">
          <div>
            ...
            <div className=" flex mb-[10px]">
              <p className=" text-gray-100 mr-[10px]">
                {movieInfo?.release_date.slice(0, 4)}
              </p>

              <FontAwesomeIcon
                icon={faCircle}
                className=" w-[5px] h-[5px] text-gray-100 mr-[10px] mt-[10px]"/>
          ...
          <div className="flex text-white">
            <div className=" mr-[20px]">
              ...
              <div className=" flex mt-[3px] ml-[10px]">
                <FontAwesomeIcon
                  className=" mt-[4px] mr-[6px] w-[25px] h-[25px]"
                  icon={faStar}
                  color="yellow"
                />
                <p className=" mt-[2px] mr-[2px] text-xl font-semibold">
                  {movieData?.imdbRating}
                </p>
                <p className=" text-base mt-[4px] text-gray-200">/10</p>
              </div>
            </div>
            ...
            <div className=" mr-[20px]">
                ...
                <div className=" flex mt-[3px] ml-[26px]">
                    <svg
                    className=" w-[25px] h-[25px] mt-[4px] mr-[6px]"
                    type="certified"
                    viewBox="0 0 80 80"
                  ...
                  ></path>
                    ...
                    <p className=" mt-[2px] mr-[2px] text-xl font-semibold" >{rottonRatingsValue ? rottonRatingsValue : "N/A"}</p>
                </div>
        ...
        <div className=" flex mb-[20px]">
          <div>
            <img
              className="w-[450px] h-[500px] max-w-[300px]"
              alt="poster"
          ...
          <div className=" h-[500px] w-[900px] ml-[30px]">
            <iframe
              ...
              src={`https://www.youtube.com/embed/${movieClip?.key}?&autoplay=1&loop=1&mute=1&playlist=${movieClip?.key}&controls=0`}
        ...
        <div className=" flex mb-[20px]">
          {movieInfo?.genres.map((n) => (
            <div className=" border border-gray-500 text-gray-100 w-[120px] h-[30px] text-center rounded-xl mr-[20px] hover:bg-gray-400 cursor-pointer">
              {n?.name}
            </div>
          ))}
        </div>
        ...
        <div>
          <p className=" text-gray-100 text-base font-medium mb-[20px]">
            {movieInfo?.overview}
          </p>
        </div>
        ...
        <div className=" flex overflow-x-scroll">
          {movieCast?.cast?.map((n) => (
            <div className=" w-[170px] h-[230px] text-center text-gray-100 mb-[10px]">
              <img
                className=" w-[110px] h-[150px] mx-auto mb-[10px]"
                alt="poster"
                src={
                  n.profile_path
                    ? `https://image.tmdb.org/t/p/w500/${n?.profile_path}`
                    : imgUrl
                }
              />
